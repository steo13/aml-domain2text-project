...
def do_eval(model, img_dataloader, phrase_dataloader, device, split, eval_p2i=True, eval_i2p=True, visualize_path=None, add_to_summary_name=None):
    # switch to infer mode
    model.eval()
    # make predictions and retrive neg_distances as <image-phrase> distance function's output
...
def main_eval():
    ...
    # by default use Bert with L2 and lr=0.0001
    parser.add_argument('-p', '--trained_path', help="path to trained model (where there is cfg file)", default='output/triplet_match/c34_bert_l2_s_lr0.00001')
    parser.add_argument('-m', '--model_file', help='file name of the cached model ', default='BEST_checkpoint.pth')
    parser.add_argument('-o', '--opts', default=None, nargs=argparse.REMAINDER, help="e.g. EVAL_SPLIT test")
    ...
    # create Texture description dataset phid_format=set by default
    dataset = TextureDescriptionData(phid_format=None)
    
    # create Image dataset and relative data loader
    img_dataset = ImgOnlyDataset(split=cfg.EVAL_SPLIT, transform=build_transforms(is_train=False), texture_dataset=dataset)
    img_dataloader = DataLoader(img_dataset, batch_size=1, shuffle=False)

    # create Phrase dataset for the above texture_dataset and relative data loader
